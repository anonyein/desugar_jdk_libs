...
/**
 * Runtime machinery for evaluating Gatherers under different modes. The performance-critical code
 * below contains some more complicated encodings: therefore, make sure to run benchmarks to verify
 * changes to prevent regressions.
 *
 * @since 24
 */
final class GathererOp<T, A, R> extends ReferencePipeline<T, R> {
  // For DESUGAR: Use Unsafe instead of VarHandle and move it outside of Hybrid non static class.
  private static final Unsafe UNSAFE = Unsafe.getUnsafe();
  private static long HYBRID_NEXT_OFFSET;

  @SuppressWarnings("unchecked")
  static <P_IN, P_OUT extends T, T, A, R> Stream<R> of(
      ReferencePipeline<P_IN, P_OUT> upstream, Gatherer<T, A, R> gatherer) {
    ...
    if (upstream.getClass() == GathererOp.class) {
      ...
      return new GathererOp<>((ReferencePipeline<?, T>) upstream, gatherer);
    }
  }
  ...
  static final class NodeBuilder<X> implements Consumer<X> {
    ...
    public NodeBuilder<X> join(NodeBuilder<X> that) {
      ...
      if (!that.isEmpty()) {
        ...
        if (rightMost != null
            && tb instanceof NodeBuilder.Builder
            && tb.count() < LINEAR_APPEND_MAX) tb.forEach(this); // Avoid conc for small nodes
        else leftMost = Nodes.conc(StreamShape.REFERENCE, this.build(), tb);
      }
    ...
    public Node<X> build() {
      if (isEmpty()) return Nodes.emptyNode(StreamShape.REFERENCE);
  ...
  private static int opFlagsFor(Integrator<?, ?, ?> integrator) {
    return integrator instanceof Integrator.Greedy<?, ?, ?> ? GREEDY_FLAGS : SHORT_CIRCUIT_FLAGS;
  }

  private static final int DEFAULT_FLAGS =
      StreamOpFlag.NOT_SORTED | StreamOpFlag.NOT_DISTINCT | StreamOpFlag.NOT_SIZED;

  private static final int SHORT_CIRCUIT_FLAGS = DEFAULT_FLAGS | StreamOpFlag.IS_SHORT_CIRCUIT;

  private static final int GREEDY_FLAGS = DEFAULT_FLAGS;
   ...
   */
  @Override
  <I> Node<R> opEvaluateParallel(
      PipelineHelper<R> unused1, Spliterator<I> spliterator, IntFunction<R[]> unused2) {
  ...
  @Override
  <P_IN> Spliterator<R> opEvaluateParallelLazy(
      PipelineHelper<R> helper, Spliterator<P_IN> spliterator) {
    /*
  ...
  @Override
  public <RR> RR collect(
      Supplier<RR> supplier, BiConsumer<RR, ? super R> accumulator, BiConsumer<RR, RR> combiner) {
    linkOrConsume(); // Important for structural integrity
    final var parallel = isParallel();
    final var u = upstream();
    return evaluate(
  ...
  private <CA, CR> CR evaluate(
    ...
    // Optimization
    final boolean greedy = integrator instanceof Integrator.Greedy;
    ...
    final class Sequential implements Consumer<T>, Gatherer.Downstream<R> {
      ...
      Sequential() {
        if (initializer != Gatherer.defaultInitializer()) state = initializer.get();
      ...
      Sequential evaluateUsing(Spliterator<T> spliterator) {
        if (greedy) spliterator.forEachRemaining(this);
        else do {} while (proceed && spliterator.tryAdvance(this));
      ...
      public CR get() {
        final var finisher = gatherer.finisher();
        if (finisher != Gatherer.<A, R>defaultFinisher()) finisher.accept(state, this);
     ...
     * strategy.
     */
    if (!parallel) return new Sequential().evaluateUsing(spliterator).get();
    ...
    final class Hybrid extends CountedCompleter<Sequential> {
      {
        if (HYBRID_NEXT_OFFSET == 0) {
            synchronized (GathererOp.class) {
              if (HYBRID_NEXT_OFFSET == 0) {
                HYBRID_NEXT_OFFSET = UNSAFE.objectFieldOffset(Hybrid.class, "next");
              }
            }
        }
      }
        ...
        while ((greedy || !cancelled.get())
          ...
          if (task.leftPredecessor != null) {
            ...
            if (UNSAFE.compareAndSetObject(
                task.leftPredecessor, HYBRID_NEXT_OFFSET, task, leftChild)) {
      ...
      public void onCompletion(CountedCompleter<?> caller) {
        ...
        if (s != null
            ...
            && !greedy) cancelled.set(true);

        // The completion of this task *and* the dumping of elements
        // "happens-before" completion of the associated left-most leaf task
        // of right subtree (if any, which can be this task's right sibling)
        @SuppressWarnings("unchecked")
        var leftDescendant = (Hybrid) UNSAFE.getAndSetObject(this, HYBRID_NEXT_OFFSET, null);
    ...
    final class Parallel extends CountedCompleter<Sequential> {
      ...
      private boolean isRequestedToCancel() {
        ...
        if (!cancel) {
          for (Parallel parent = getParent();
              !cancel && parent != null;
              parent = parent.getParent()) cancel = parent.canceled;
        }
        return cancel;
      }
      ...
      private void cancelLaterTasks() {
        ...
        for (Parallel parent = getParent(), node = this;
          ...
          if (parent.leftChild == node) parent.rightChild.canceled = true;
        }
      }
    }

    if (combiner != Gatherer.defaultCombiner()) {
      return new Parallel(spliterator).join().get();
    } else {
      return new Hybrid(spliterator).join().get();
    }
  }
}
