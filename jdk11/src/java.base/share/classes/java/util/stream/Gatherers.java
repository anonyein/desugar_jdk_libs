 ...
 * windowing functions, folding functions, transforming elements concurrently, etc.
 *
 * @since 24
 */
public final class Gatherers {
  ...
  /**
   * Returns a Gatherer that gathers elements into windows -- encounter-ordered groups of elements
   * -- of a fixed size. If the stream is empty then no window will be produced. The last window may
   * contain fewer elements than the supplied window size.
   *
   * <p>Example:
   *
   * {@snippet lang = java:
   ...
   * }
   *
   * @implSpec Each window produced is an unmodifiable List; calls to any mutator method will always
   *     cause {@code UnsupportedOperationException} to be thrown. There are no guarantees on the
   *     implementation type or serializability of the produced Lists.
   * @apiNote For efficiency reasons, windows may be allocated contiguously and eagerly. This means
   *     that choosing large window sizes for small streams may use excessive memory for the
   *     duration of evaluation of this operation.
   * @param windowSize the size of the windows
   * @param <TR> the type of elements the returned gatherer consumes and the contents of the windows
   *     it produces
    ...
    class FixedWindow {
      ...
      boolean integrate(TR element, Downstream<? super List<TR>> downstream) {
        ...
        if (at < windowSize) {
          ...
          window = new Object[windowSize];
          at = 0;
          // FOR DESUGAR: Replace with Arrays.asList().
  ...
  /**
   * Returns a Gatherer that gathers elements into windows -- encounter-ordered groups of elements
   * -- of a given size, where each subsequent window includes all elements of the previous window
   * except for the least recent, and adds the next element in the stream. If the stream is empty
   * then no window will be produced. If the size of the stream is smaller than the window size then
   * only one window will be produced, containing all elements in the stream.
   *
   * <p>Example:
   *
   * {@snippet lang = java:
  ...
  /**
   * Returns a Gatherer that performs an ordered, <i>reduction-like</i>, transformation for
   * scenarios where no combiner-function can be implemented, or for reductions which are
   * intrinsically order-dependent.
   *
   * @implSpec If no exceptions are thrown during processing, then this operation only ever produces
   *     a single element.
   *     <p>Example:
   *     {@snippet lang = java:
  ...
  public static <T, R> Gatherer<T, ?, R> fold(
      Supplier<R> initial, BiFunction<? super R, ? super T, ? extends R> folder) {
  ...
  /**
   * Returns a Gatherer that performs a Prefix Scan -- an incremental accumulation -- using the
   * provided functions. Starting with an initial value obtained from the {@code Supplier}, each
   * subsequent value is obtained by applying the {@code BiFunction} to the current value and the
   * next input element, after which the resulting value is produced downstream.
   *
   * <p>Example:
   *
   * {@snippet lang = java:
  ...
  public static <T, R> Gatherer<T, ?, R> scan(
      Supplier<R> initial, BiFunction<? super R, ? super T, ? extends R> scanner) {
    Objects.requireNonNull(initial, "'initial' must not be null");
  ...
  /**
   * An operation which executes a function concurrently with a configured level of max concurrency,
   * using <a href="{@docRoot}/java.base/java/lang/Thread.html#virtual-threads">virtual threads</a>.
   * This operation preserves the ordering of the stream.
   *
   * @apiNote In progress tasks will be attempted to be cancelled, on a best-effort basis, in
   *     situations where the downstream no longer wants to receive any more elements.
   * @implSpec If a result of the function is to be pushed downstream but instead the function
   *     completed exceptionally then the corresponding exception will instead be rethrown by this
   *     method as an instance of {@link RuntimeException}, after which any remaining tasks are
   *     canceled.
  ...
  public static <T, R> Gatherer<T, ?, R> mapConcurrent(
      final int maxConcurrency, final Function<? super T, ? extends R> mapper) {
    if (maxConcurrency < 1)
    ...
    final class State {
      ...
      boolean flush(long atLeastN, Downstream<? super R> downstream) {
        ...
        try {
          ...
          // FOR DESUGAR: Make jdk11 compliant.
          ...
          if (!success && !wip.isEmpty()) {
            ...
            for (var task : wip) task.cancel(true);
  ...
  enum Value implements Supplier, BinaryOperator, BiConsumer {
    ...
    final BinaryOperator<Void> statelessCombiner =
        new BinaryOperator<>() {
          @Override
  ...
  static final class Composite<T, A, R, AA, RR> implements Gatherer<T, Object, RR> {
    ...
    static <T, A, R, AA, RR> Composite<T, A, R, AA, RR> of(
        Gatherer<T, A, ? extends R> left, Gatherer<? super R, AA, ? extends RR> right) {
    ...
    static final <T, A, R, AA, RR> GathererImpl<T, ?, RR> impl(
      ...
      if (leftStateless && rightStateless && leftGreedy && rightGreedy) {
            ...
            Gatherer.Integrator.ofGreedy(
                (unused, element, downstream) ->
                    leftIntegrator.integrate(
                        null, element, r -> rightIntegrator.integrate(null, r, downstream))),
        ...
        class State {
          ...
          State() {
            this(
                leftStateless ? null : leftInitializer.get(),
